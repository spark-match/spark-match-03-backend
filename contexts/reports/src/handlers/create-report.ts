...
//
// No request body. The owner comes from the JWT, never from the payload --
// otherwise anyone could open a report in someone else's name.
//
...
export const handler = buildHandler<unknown, Report>({
  ...
  handler: async (_input, _event, auth) => {
    ...
    const informe = await ctx.reportService.request({ userId: auth.userId });
