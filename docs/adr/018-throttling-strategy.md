...
|---|---|---|---|---|
| **Edge throttling** (`AWS::ApiGatewayV2::Route ThrottleSettings`) | Per-route o per-stage | **Gratis** (incluido) | <1ms | DDoS, abuse global |
...
- Per-account lockout tiene riesgo de DoS legítimo (atacante lockea cuentas ajenas con intentos fallidos). Mitigable: heurística Layer 3 más sofisticada en futuro (e.g., lockout también si password es **muy obvia** vs muchos intentos con password distinta).
...
- WAF Per-IP (Layer intermedio) — deferred hasta que el tráfico lo justifique.
