...
Hoy no hay rate limiting implementado. El código de error `too_many_requests` (HTTP 429) está definido en `error-catalog.md:181` pero nunca se lanza — el comentario dice "Rate limiting (futuro)".
...
### PR-1 (Layer 1) — API Gateway throttling — **XS, ~1 día, ya propuesto**
...
### PR-2 (Layer 3) — App-level account lockout — **S, ~1 sprint, deferrable**
...
- AuthLoginRoute:
    RouteSelectionExpression: '$request.path'
    Target: integrations/login
    ThrottleSettings:
      RateLimit: 5.0
      BurstLimit: 10

# resto usa default global
```
...
- `docs/error-catalog.md` línea 181: `too_many_requests` (429) ya definido pero nunca emitido.