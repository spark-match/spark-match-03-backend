...
| B19 | ESLint uses `tseslint.configs.recommended` without `parserOptions.project`, so type-aware rules SonarCloud may flag are not caught locally. | **Closed by PR #179** (switched to `recommendedTypeChecked` + dedicated `tsconfig.eslint.json`; high-value rules enabled: `no-floating-promises`, `no-misused-promises`, `await-thenable`, `return-await`; `no-unnecessary-condition` + `require-await` disabled as noise-on-Middy). |
| B26 | **Standing CodeQL alert on `.github/workflows/deploy.yml`** for `actions/unpinned-tag` on third-party action pins. Detection contradicts §12.1, which mandates `@vN` and forbids SHA-pinning. `01-devops` suppresses the identical rule for itself in `.github/codeql/codeql-config.yml` and now exposes the `config-file:` input that backend needs (01-devops PR #290, release v1.1.0). Backend mirrors the exclusion in `.github/codeql/codeql-config.yml` (PR #140) and forwards it via `config-file:` in `.github/workflows/codeql.yml`. **Caveat (2026-08-04, see `docs/audits/codeql-config-exclusion-2026-08-04.md`):** CodeQL CLI v2.26.2 + `actions-queries` v0.6.32 (via `github/codeql-action/init@v4.37.4`) parses `query-filters: - exclude: id:` but **does not prevent query evaluation** — the excluded `UnpinnedActionsTag.ql` still runs and emits SARIF alerts. Workaround attempt in PR #162 (declaring `queries: - uses: security-extended` in the config) did NOT change the behaviour (same log evidence). On 2026-08-04, 3 alerts (#6 deploy.yml:93 `spark-match-01-devops/...@main`, #8 deploy.yml:79 `aws-actions/setup-sam@v3`, #9 deploy.yml:113 `aws-actions/configure-aws-credentials@v6`) were dismissed as `won't fix` with §13 B26 + §12.1 justifications. **Operative defense:** `tests/bats/no-sha-pinning.bats` (PR #138), which rejects SHA-pinned uses at PR-time. Closing PR #140 remains correct (config-file forwarding still useful for future CodeQL versions + documents intent); the manual dismissal loop is the practical workaround until upstream fixes the filter. |
...
| B21 | `pull_request_template.md` references `contexts/assessment|career|matching`, `events/`, and `docs/CHANGELOG.md` — none exist. It has no reviewer checklist and no cross-repo PR link field (both present in `02-infrastructure`). | **Closed by PR #177** (nonexistent paths removed: `contexts/assessment|career|matching`, `events/`, `docs/CHANGELOG.md` → `CHANGELOG.md`; reviewer checklist and cross-repo PR link field added, matching 02-infrastructure). |
...
| B23 | `codeql.yml` header comments are stale (say "weekly Monday 06:17 UTC" vs the actual daily `0 8 * * *`; reference `codeql.yml@main` vs the actual `reusable-codeql.yml@main`). | **Closed by PRs #114, #126, #127** (cron synced to `0 8 * * *`, header ref corrected to `reusable-codeql.yml@main`). |
...
| B25 | The `yamllinks` job id in `ci.yml:77` is a typo for `yamllint`, and violates §12.1's brand-spelling rule. | **Closed by PR #173** (job id renamed `yamllinks` → `yamllint`). |
